...
const ImmudbClient = (config) => {
  // TODO(andrew): TLS config.
  client = new services.ImmuServiceClient(config.address,
  ...
  client.printTree(req, metadata, (err, res) => {
    ...
    for (let i = 0; i < tl.length; i++) {
      ...
      for (let j = 0; j < ll.length; j++) {
        ...
        nodes.push({
          h: util.base64Decode(node.getH()),
          i: util.base64Decode(node.getI()),
          refk:  refk,
          ref: node.getRef(),
          cache: node.getCache(),
          root: node.getRoot(),
...
const currentRoot = (params, callback) => {
  ...
  client.currentRoot(req, metadata, (err, res) => {
    ...
    callback(null, {
      index: res.getIndex(),
      signature: util.base64Decode(res.getRoot()),
    })
  })
}
...
const safeSet = (params, callback) => {
  ...
  client.safeSet(req, metadata, (err, res) => {
    ...
    proofs.verify(verifyReq, (err) => {
      ...
      callback(null, {
        index: res.getIndex(),
        root: util.base64Decode(res.getRoot()),
      })
    })
  })
}
