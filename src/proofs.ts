...
class Proof {
    ...
    _digest (params: any) {
        ...
        let buf: any = Buffer.alloc(1 + 8 + 8 + kl + vl);
    ...
    verifyInclusion (params: any, callback: any) {
        ...
        for (let i = 0; i < path.length; i++) {
            ...
            let c: any = Buffer.alloc(this.sha256Size * 2 + 1);
            c[0] = this.nodePrefix;
    ...
    verifyConsistency (params: any, callback: any) {
        ...
        if (params.proof.at == params.oldRoot.index &&
            this.util && this.util.equalArray(new Uint8Array(oldHash),
        ...
        let tmp: any = Buffer.alloc(this.sha256Size*2 + 1);
        ...
        if (this.util && !this.util.equalArray(new Uint8Array(fr),
            new Uint8Array(oldHash)) ||
            (this.util && !this.util.equalArray(new Uint8Array(sr),
