...
tap.test('database management', async (t) => {
  ...
  let immudbClient = await ImmudbClient.getInstance(config);
  try {
    // test: login using the specified username and password
    const firstRequestData: messages.LoginRequest.AsObject = { user: IMMUDB_USER, password: IMMUDB_PWD}
    ...
    const fourtResponse = await immudbClient.set(fourthRequestData)
    if (fourtResponse) {
      t.equal(fourtResponse.index, 0)
    }
    else {
    ...
    const fifthResponse: messages.DatabaseListResponse.AsObject | undefined = await immudbClient.listDatabases()
    if (fifthResponse) {
...
tap.test('user management', async (t) => {
  ...
  try {
    ...
    // test: login using the specified username and password
    let loginRequest: messages.LoginRequest.AsObject = { user: IMMUDB_USER, password: IMMUDB_PWD}
    let loginResponse = await immudbClient.login(loginRequest)
    if (loginResponse) {
...
tap.test('operations', async (t) => {
  ...
  //     * Math.floor(100000))
    ...
    // test: login using the specified username and password
    let loginRequest: messages.LoginRequest.AsObject = { user: IMMUDB_USER, password: IMMUDB_PWD}
    const loginResponse: messages.LoginResponse.AsObject | undefined = await immudbClient.login(loginRequest)
    ...
    // and value
    let setRequest: messages.KeyValue.AsObject = { key: new Uint8Array(rand), value: new Uint8Array(rand) }
...
tap.test('batches', async (t) => {
  ...
  try {
    // test: login using the specified username and password
    const loginRequest: messages.LoginRequest.AsObject = { user: IMMUDB_USER, password: IMMUDB_PWD}
    let res = await immudbClient.login(loginRequest)
    ...
    for (let i = 0; i < 20; i++) {
      setBatchRequest.kvsList.push({ key: new Uint8Array(i), value: new Uint8Array(i) })
    }
    let setBatchResponse = await immudbClient.setBatch(setBatchRequest)

    // test: execute a batch read
    const getBatchRequest: messages.KeyList.AsObject = { keysList : [] }
    for (let i = 0; i < 20; i++) {
      getBatchRequest.keysList.push({ key: new Uint8Array(i) })
    }
    const getBatchResponse = await immudbClient.getBatch(getBatchRequest)
